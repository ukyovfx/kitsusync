...
func TestRuntimeKitsuDataSourcePrefersDecryptablePersistedTokenWithoutEnv(t *testing.T) {
	t.Setenv(RuntimeSecretKeyFileEnv, filepath.Join(t.TempDir(), "runtime-secret.key"))
	...
	if !ok || token != "persisted-runtime-token" || base != "https://kitsu.example.test/api" {
		t.Fatalf("runtime data source = base %q token_present=%t ok=%t", base, token != "", ok)
	}
}

func TestRuntimeKitsuDataSourceFailsClosedForUnreadablePersistedToken(t *testing.T) {
	t.Setenv(RuntimeSecretKeyFileEnv, filepath.Join(t.TempDir(), "runtime-secret.key"))
	t.Setenv("KitsuJWTToken", "stale-env-token")
	db := newRuntimeCredentialTestDB(t)
	model.SetSetting(db, KitsuAPIBaseURLSettingKey, "https://kitsu.example.test/api")
	...
	if base, token, ok := runtimeKitsuDataSource(db); ok || base != "" || token != "" {
		t.Fatalf("unreadable persisted credential was not fail-closed: base=%q token_present=%t ok=%t", base, token != "", ok)
	}
}

func TestRuntimeKitsuDataSourceWithoutCredentialIsUnavailable(t *testing.T) {
	t.Setenv("KitsuJWTToken", "")
	if base, token, ok := runtimeKitsuDataSource(nil); ok || base != "" || token != "" {
		t.Fatalf("missing runtime credential was not blocked: base=%q token_present=%t ok=%t", base, token != "", ok)
	}
}
