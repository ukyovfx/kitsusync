...
func renderProjectDeliveryCard(lang string, state projectDeliveryState) string {
	...
	if len(options) == 0 {
		options = append(options, `<option value="" data-project-name="" data-summary="" data-detail="" data-last-verified="-" data-pill-class="warn" data-can-send="false">No productions</option>`)
	}
  ...
  button.addEventListener('click', async function(){
    ...
    try {
      ...
      button.textContent = original;
		...
		esc(t(lang, "騾夂衍驟堺ｿ｡縺ｮ遒ｺ隱・, "Notification delivery verification")),
		esc(state.Detail),
		fixHTML,
		pillClass,
		esc(state.Summary),
		...
		esc(selectedDetail),
		esc(t(lang, "譛邨ら｢ｺ隱・ ", "Last verified: ")),
		esc(selectedLastVerified),
		selectedPillClass,
		esc(selectedSummary),
		esc(selectedProjectID),
		map[bool]string{true: "", false: " disabled"}[selectedCanSend],
		esc(t(lang, "繝・せ繝磯夂衍繧帝∽ｿ｡", "Send test notification")),
		t(lang, "譛邨ら｢ｺ隱・ ", "Last verified: "),
		t(lang, "騾∽ｿ｡荳ｭ...", "Sending..."),
		state.APIPath,
		t(lang, "遒ｺ隱肴ｸ医∩", "Verified"),
		t(lang, "縺薙・ project 縺ｮ繝・せ繝磯夂衍謌仙粥縺瑚ｨ倬鹸縺輔ｌ縺ｾ縺励◆縲・, "A successful test notification was recorded for this project."),
		t(lang, "遒ｺ隱肴ｸ医∩", "Verified"),
		t(lang, "縺薙・ project 縺ｮ繝・せ繝磯夂衍謌仙粥縺瑚ｨ倬鹸縺輔ｌ縺ｾ縺励◆縲・, "A successful test notification was recorded for this project."),
		t(lang, "譛邨ら｢ｺ隱・ ", "Last verified: "),
		t(lang, "繝・せ繝磯夂衍繧帝∽ｿ｡縺励∪縺励◆縲・iscord 蛛ｴ縺ｮ逹蠑ｾ繧ら｢ｺ隱阪＠縺ｦ縺上□縺輔＞縲・, "A test notification was sent. Confirm that it arrived in Discord as well."),
		t(lang, "譛ｪ遒ｺ隱・, "Not verified"),
		t(lang, "繝・せ繝磯夂衍繧帝∽ｿ｡縺ｧ縺阪∪縺帙ｓ縺ｧ縺励◆: ", "The test notification could not be sent: "),
	)
}
