...
func renderConnectionsEditFormWithIdentityRows(lang string, r *http.Request, db *gorm.DB, statusHint, statusClass, statusLabel, runtimeHost string, kitsuHealthy, discordHealthy bool, discordBotName string) string {
	...
	endpointControl := `<div class="connection-form-field connection-host-field" data-kitsu-endpoint-auto><label for="kitsu-hostname-resolved">` + esc(tr(lang, "connections.host")) + `</label><div class="connection-host-value-row"><code id="kitsu-hostname-resolved">` + esc(displayHost) + `</code><input type="hidden" id="kitsu-hostname-auto" name="kitsu_hostname" form="kitsu-connection-form" value="` + esc(resolvedHost) + `" data-kitsu-host-auto><button type="button" class="btn-ghost connection-endpoint-change" data-reveal-kitsu-endpoint>` + esc(t(lang, "手動変更", "Change manually")) + `</button></div><p class="field-help" data-kitsu-host-auto-help>` + esc(t(lang, "自動検出または保存済み設定から使用しています。", "Detected automatically or loaded from the saved configuration.")) + `</p></div><div class="connection-form-field connection-host-manual" data-kitsu-endpoint-manual hidden><label for="kitsu-hostname">` + esc(tr(lang, "connections.host")) + `</label><input id="kitsu-hostname" type="url" name="kitsu_hostname" form="kitsu-connection-form" value="` + esc(resolvedHost) + `" aria-describedby="kitsu-host-help" data-initial-value="` + esc(resolvedHost) + `" disabled><p id="kitsu-host-help" class="field-help">` + esc(tr(lang, "connections.host_help")) + `</p><button type="button" class="btn-ghost connection-endpoint-reset" data-reset-kitsu-endpoint>` + esc(t(lang, "自動設定に戻す", "Use automatic endpoint")) + `</button></div>`
	if resolvedHost == "" {
		endpointControl = `<div class="connection-form-field connection-host-manual" data-kitsu-endpoint-manual><label for="kitsu-hostname">` + esc(tr(lang, "connections.host")) + `</label><input id="kitsu-hostname" type="url" name="kitsu_hostname" form="kitsu-connection-form" value="" aria-describedby="kitsu-host-help" data-initial-value=""><p id="kitsu-host-help" class="field-help">` + esc(tr(lang, "connections.host_help")) + `</p></div>`
	}
