...
func renderConnectionsEditFormWithIdentityRows(lang string, r *http.Request, db *gorm.DB, statusHint, statusClass, statusLabel, runtimeHost string, kitsuHealthy, discordHealthy bool, discordBotName string) string {
	...
	advancedSettings := `<section class="editorial-advanced-settings" aria-labelledby="connections-advanced-title"><div class="page-heading"><div><h2 id="connections-advanced-title">` + esc(t(lang, "詳細設定", "Advanced settings")) + `</h2><p class="field-help">` + esc(t(lang, "特殊な経路や通知リンクでのみ使用する設定です。", "Only needed for exceptional routing or notification-link setups.")) + `</p></div></div>` + externalURLField + `<div class="connection-form-field"><label for="kitsu-internal-url">` + esc(t(lang, "内部 Kitsu URL", "Internal Kitsu URL")) + `</label><input id="kitsu-internal-url" type="url" name="kitsu_internal_url" autocomplete="url"></div><div class="connection-form-field"><label for="kitsu-api-base-url">` + esc(t(lang, "API Base URL", "API Base URL")) + `</label><input id="kitsu-api-base-url" type="url" name="kitsu_api_base_url" value="` + esc(model.GetSetting(db, KitsuAPIBaseURLSettingKey)) + `" autocomplete="url"><p class="field-help">` + esc(t(lang, "Kitsu URLとAPIの起点が異なる特殊なリバースプロキシ用です。", "Only needed for unusual reverse proxy setups where the API is not under the Kitsu URL.")) + `</p></div></section>`
