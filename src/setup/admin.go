...
func renderConnectionsEditFormWithIdentityRows(lang string, r *http.Request, db *gorm.DB, statusHint, statusClass, statusLabel, runtimeHost string, kitsuHealthy, discordHealthy bool, discordBotName string) string {
	...
	externalURLField := `<div class="connection-form-field"><label for="kitsu-external-url">` + esc(t(lang, "外部Kitsu URL（任意）", "External Kitsu URL (optional)")) + `</label><input id="kitsu-external-url" type="url" name="kitsu_external_url" form="kitsu-connection-form" value="` + esc(externalURL) + `" aria-describedby="kitsu-external-url-help"><p id="kitsu-external-url-help" class="field-help">` + esc(t(lang, "Discord通知のKitsuリンクに使用するURLです。未設定時はKitsu URLを使用します。", "The URL used for Kitsu links in Discord notifications. When empty, the Kitsu URL is used.")) + `</p>` + checkLink + `</div>`
	expertOpen := ""
	if apiBaseURL != "" {
		expertOpen = " open"
	}
	expertOverrides := `<details class="connection-expert-network" data-expert-network-overrides` + expertOpen + `><summary>` + esc(t(lang, "専門ネットワーク設定", "Expert network overrides")) + `</summary><div class="connection-expert-network-content"><div class="connection-form-field" data-kitsu-internal-endpoint hidden><label for="kitsu-internal-url">` + esc(t(lang, "内部 Kitsu URL", "Internal Kitsu URL")) + `</label><input id="kitsu-internal-url" type="url" name="kitsu_internal_url" form="kitsu-connection-form" autocomplete="url"><p class="field-help">` + esc(t(lang, "KitsuSyncの実行先を通常のKitsu URLと分ける場合だけ使用します。", "Use only when KitsuSync must reach a different internal endpoint than the normal Kitsu URL.")) + `</p></div><div class="connection-form-field"><label for="kitsu-api-base-url">` + esc(t(lang, "API Base URL", "API Base URL")) + `</label><input id="kitsu-api-base-url" type="url" name="kitsu_api_base_url" form="kitsu-connection-form" value="` + esc(apiBaseURL) + `" autocomplete="url"><p class="field-help">` + esc(t(lang, "Kitsu URLとAPIの起点が異なる特殊なリバースプロキシ用です。", "Only needed for unusual reverse proxy setups where the API is not under the Kitsu URL.")) + `</p></div></div></details>`
	advancedSettings := `<section class="editorial-advanced-settings" aria-labelledby="connections-advanced-title"><div class="page-heading"><div><h2 id="connections-advanced-title">` + esc(t(lang, "詳細設定", "Advanced settings")) + `</h2><p class="field-help">` + esc(t(lang, "通知リンクに必要な外部URLだけを通常設定として表示します。", "Only the External Kitsu URL used in notification links is shown as a normal setting.")) + `</p></div></div>` + externalURLField + expertOverrides + `</section>`
	endpointControl := `<div class="connection-resolved-endpoint"><span class="connection-resolved-endpoint-label">` + esc(t(lang, "解決済み Kitsu endpoint", "Resolved Kitsu endpoint")) + `</span><code>` + esc(displayHost) + `</code><p class="field-help">` + esc(t(lang, "自動検出または保存済み設定から使用しています。", "Detected automatically or loaded from the saved configuration.")) + `</p><button type="button" class="btn-ghost connection-endpoint-change" data-reveal-kitsu-endpoint>` + esc(t(lang, "endpoint を手動変更", "Change endpoint manually")) + `</button></div><div class="connection-form-field" data-kitsu-endpoint-field hidden><label for="kitsu-hostname">` + esc(tr(lang, "connections.host")) + `</label><input id="kitsu-hostname" type="url" name="kitsu_hostname" value="` + esc(resolvedHost) + `" aria-describedby="kitsu-host-help" data-initial-value="` + esc(resolvedHost) + `"><p id="kitsu-host-help" class="field-help">` + esc(tr(lang, "connections.host_help")) + `</p></div>`
	if resolvedHost == "" {
		endpointControl = `<div class="connection-form-field" data-kitsu-endpoint-field><label for="kitsu-hostname">` + esc(tr(lang, "connections.host")) + `</label><input id="kitsu-hostname" type="url" name="kitsu_hostname" value="" aria-describedby="kitsu-host-help" data-initial-value=""><p id="kitsu-host-help" class="field-help">` + esc(tr(lang, "connections.host_help")) + `</p></div>`
	}
	body := `<div class="section-stack connections-edit-stack">` + notice +
		...
		`</div>` + advancedSettings + `<div class="button-row connections-navigation"><a class="btn-ghost" href="` + esc(withLang("/bot/admin/projects", r)) + `">` + esc(t(lang, "プロダクション一覧へ戻る", "Back to Productions")) + `</a></div><script>(function(){var saveText='` + esc(t(lang, "変更を保存", "Save changes")) + `';document.querySelectorAll('[data-reveal-secret]').forEach(function(button){button.addEventListener('click',function(){var input=document.getElementById(button.getAttribute('data-reveal-secret'));var submit=document.getElementById(button.getAttribute('data-recheck-button'));if(!input)return;input.hidden=false;input.style.display='';button.hidden=true;if(submit)submit.textContent=saveText;input.focus();});});var endpointButton=document.querySelector('[data-reveal-kitsu-endpoint]');if(endpointButton){endpointButton.addEventListener('click',function(){var field=document.querySelector('[data-kitsu-endpoint-field]');var internal=document.querySelector('[data-kitsu-internal-endpoint]');if(field){field.hidden=false;var input=field.querySelector('input');if(input)input.focus();}if(internal)internal.hidden=false;endpointButton.hidden=true;});}var host=document.getElementById('kitsu-hostname');var hostSubmit=document.getElementById('kitsu-recheck');if(host&&hostSubmit){host.addEventListener('input',function(){if(host.value!==host.getAttribute('data-initial-value'))hostSubmit.textContent=saveText;});}}());</script></div>`
	return body
}
