...
func TestDryRunActionIsLocalAndRedactsDestinationURL(t *testing.T) {
	...
	if err := db.AutoMigrate(&model.Project{}, &model.ProjectWebhook{}, &model.ProductionNotificationConfig{}, &model.ProductionNotificationRoute{}, &model.NotificationRoutingDiagnosis{}); err != nil { t.Fatal(err) }
	if err := model.CreateProject(db, "p1", "P", "", "", "", ""); err != nil { t.Fatal(err) }
	if err := model.CreateProjectWebhook(db, "p1", "Alerts", "", "https://example.invalid/secret-value", "11111111111111111"); err != nil { t.Fatal(err) }
	webhook := model.ListProjectWebhooks(db, "p1")[0]
	if err := model.SaveProductionNotificationConfig(db, &model.ProductionNotificationConfig{ProductionID: "p1", Enabled: true}, []model.ProductionNotificationRoute{{TaskTypeID: "tt1", DestinationWebhookID: webhook.ID}}); err != nil { t.Fatal(err) }
	...
	if !strings.Contains(body, "Dry-run") || !strings.Contains(body, "Production ID: p1") || !strings.Contains(body, "Task Type ID: tt1") { t.Fatal("dry-run result was not rendered") }
	if strings.Contains(body, "secret-value") { t.Fatal("dry-run rendered webhook URL") }
}
