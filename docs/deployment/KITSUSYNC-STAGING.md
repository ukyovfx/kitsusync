...
Candidate implementation, focused tests, CI/Security checks and immutable artifact provenance precede staging deployment. `scripts/deploy-kitsusync-staging-candidate.ps1 -CommitSha <sha>` is the only operator input: it finds a successful CI run for that exact SHA, downloads that run's exact candidate artifact into a fresh protected temporary directory, verifies the GitHub ZIP digest, candidate provenance, expected file set and recorded file digests, and only then stages the verified bundle and invokes the host helper through `sudo -n`. It removes its temporary files on exit and never accepts a caller-supplied bundle directory. Deployment never promotes the artifact to release identity. Human review occurs on port 18091. Only explicit human acceptance can authorize merge, release, or Production deployment.
