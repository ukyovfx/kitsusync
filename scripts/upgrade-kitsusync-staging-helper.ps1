...
function Get-StagingHelperUpgradeAction([int]$Status, [string]$Output) {
    $currentContract = 'STAGING_HELPER_CONTRACT=staging-v7 incoming=/var/tmp/kitsusync-staging-candidate-<sha> owners=ukyo_vfx,vfx-breakglass'
    $predecessorContracts = @(
        'STAGING_HELPER_CONTRACT=staging-v6 incoming=/var/tmp/kitsusync-staging-candidate-<sha> owners=ukyo_vfx,vfx-breakglass',
...
'@
    ...
    if ($LASTEXITCODE -ne 0 -or ($installed -join "`n") -notmatch 'STAGING_HELPER_CONTRACT=staging-v7 incoming=/var/tmp/kitsusync-staging-candidate-<sha> owners=ukyo_vfx,vfx-breakglass') {
