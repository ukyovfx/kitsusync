...
if ((Get-StagingHelperUpgradeAction 0 $v4) -ne 'ALREADY_CURRENT') { throw 'v4 helper was not recognized as current.' }
...
foreach ($case in @(
    ...
    @{ Status = 0; Output = 'STAGING_HELPER_CONTRACT=staging-v5 incoming=/var/tmp/kitsusync-staging-candidate-<sha> owners=ukyo_vfx,vfx-breakglass'; Name = 'unknown-version' },
