...
grep -Fxq "version=${version}" "${tmp}/pr"
...
GITHUB_EVENT_NAME=push GITHUB_REF="refs/tags/${release_tag}" GITHUB_SHA="${a}" GITHUB_OUTPUT="${tmp}/release" bash "${resolver}"
...
ARTIFACT_KIND=candidate SOURCE_COMMIT="${b}" SOURCE_ID="${b}" MERGE_TEST_COMMIT="${a}" RELEASE_VERSION="${version}" IMAGE_ID="${image}" PROVENANCE_OUTPUT="${tmp}/manifest" bash "${generator}"
...
ARTIFACT_KIND=release SOURCE_COMMIT="${a}" SOURCE_ID="${a}" RELEASE_COMMIT="${a}" RELEASE_VERSION="${version}" RELEASE_TAG="${release_tag}" IMAGE_ID="${image}" IMAGE_REF="kitsusync:${release_tag}" IMAGE_ARCHIVE_SHA256="${digest}" IMAGE_CONFIG_DIGEST="${portable}" IMAGE_MANIFEST_DIGEST="${portable}" IMAGE_CONTENT_DIGEST="${portable}" COMPOSE_SHA256="${digest}" DEPLOYMENT_TOOL_SHA256="${digest}" PREVIEW_DEPLOYMENT_TOOL_SHA256="${digest}" DEPLOYMENT_CORE_SHA256="${digest}" INSPECTION_TOOL_SHA256="${digest}" SQLITE_BACKUP_TOOL_SHA256="${digest}" BOOTSTRAP_TOOL_SHA256="${digest}" IMAGE_IDENTITY_TOOL_SHA256="${digest}" RUNTIME_STATE_TOOL_SHA256="${digest}" RESTORE_STATE_TOOL_SHA256="${digest}" PROVENANCE_OUTPUT="${tmp}/release-manifest" bash "${generator}"
...
grep -Fxq "release_tag=${release_tag}" "${tmp}/release-manifest"
grep -Fxq "image_ref=kitsusync:${release_tag}" "${tmp}/release-manifest"
