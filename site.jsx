...
const sections = {
  ja: [
    ...
    { id: 'routes', num: '02', label: '画面・URL', kicker: 'Pages & Routes', title: '画面・URL', subtitle: '目的ごとの入口', table: { headers: ['URL', '用途'], rows: [['/bot/login', '管理画面へのログイン'], ['/bot/setup', '新しいプロダクションの接続ウィザード'], ['/bot/admin', 'ダッシュボードと管理メニュー'], ['/bot/admin/bot', 'Kitsu接続とDiscord Bot接続'], ['/bot/admin/projects', 'プロダクション一覧とプロダクション詳細'], ['/bot/admin/users', 'KitsuユーザーとDiscord IDのユーザー紐づけ'], ['/bot/admin/checkers', 'Checker / Reviewerの割り当て'], ['/bot/admin/health', 'システム状態、API応答、運用診断'], ['/bot/admin/audit', '監査ログとDaily Summary'], ['/bot/docs/', 'この公式ドキュメント'] ] } },
    ...
    { id: 'channels', num: '04', label: '通知チャンネル', kicker: 'Notification Routing', title: 'プロダクションの通知ルーティング', subtitle: 'Kitsu Task TypeからDiscord チャンネルへ', intro: 'プロダクション詳細の「通知」では、Kitsu Task TypeとDiscord チャンネルの対応を確認できます。通常は読み取り専用で表示し、編集するときだけ「編集」を選びます。', bullets: ['ルーティングの識別にはプロダクション IDと安定したTask Type IDを使います。表示名だけでは送信先を決めません。', '編集では、既存ルートの順序変更、送信先変更、ルート追加、通知対象からの除外をまとめて確認してから「変更を適用」します。', '「通知対象から外す」はルートだけを外し、Kitsu Task TypeやDiscord チャンネルを削除しません。', 'Discordチャンネルを削除する場合は、所有確認、権限確認、チャンネル名の入力が必要です。', 'チャンネルやWebhookが見つからない、所有情報が曖昧、権限が不足している場合は推測せず、修復または診断画面を使います。'] },
  ...
  en: [
    ...
    { id: 'routes', num: '02', label: 'Pages & Routes', kicker: 'Pages & Routes', title: 'Pages & Routes', subtitle: 'Where to go for each task', table: { headers: ['URL', 'Purpose'], rows: [['/bot/login', 'Sign in to the admin area'], ['/bot/setup', 'New Production Connection wizard'], ['/bot/admin', 'Dashboard and management menu'], ['/bot/admin/bot', 'Kitsu and Discord Bot connections'], ['/bot/admin/projects', 'Production list and Production details'], ['/bot/admin/users', 'Link Kitsu people to Discord IDs'], ['/bot/admin/checkers', 'Assign Checker / Reviewer roles'], ['/bot/admin/health', 'System Status, API response, and diagnostics'], ['/bot/admin/audit', 'Audit Log and Daily Summary'], ['/bot/docs/', 'This official documentation'] ] } },
...
const guidance = {
  ja: {
    ...
    routes: { purpose: '管理画面の各ページを、目的から探せるようにします。', when: 'どの画面を開けばよいか分からないときに使います。', steps: ['管理者ログイン後、ダッシュボードから目的に合うページを開きます。', '接続は「接続設定」、プロダクションは「プロダクション一覧」、通知確認は「システム状態」または「監査ログ」を選びます。'], after: '選んだページで、現在の設定や状態だけを確認・変更できます。', notes: ['管理画面のページはログインが必要です。', 'このドキュメントのURLは読み取り専用です。'], check: 'ログイン画面に戻る場合はセッションが切れています。もう一度ログインしてから元のURLを開きます。' },
    ...
    preview: { purpose: '通知を送る前に、送信先と表示内容を確認します。', when: 'ルートや通知文面を確認したいときに使います。', steps: ['プロダクション詳細の通知プレビューでTask Typeを選びます。', '送信先Discord チャンネル、通知言語、メンションの有無を確認します。', '表示されたDiscordメッセージの内容とKitsuリンクを確認します。'], after: 'プレビューは内容を表示するだけで、Discordへメッセージを送りません。', notes: ['KitsuのタスクIDと設定済みホストが確認できる場合だけリンクを表示します。', '保存先リンクや画像は、設定済みまたはKitsuから取得できる情報だけを使います。'], check: '送信先が空の場合は「通知ルーティング」を、リソースが古い場合は「トラブルシューティング」を確認します。' },
  ...
  en: {
    ...
    routes: { purpose: 'This section helps you find the correct KitsuSync page for each task.', when: 'Use it when you are unsure where to change a setting or inspect a result.', steps: ['Sign in and start from Dashboard.', 'Use Connections for services, Production list for Production details, and System Status or Audit Log for notification checks.'], after: 'The selected page shows the settings or status for that task.', notes: ['Admin pages require an authenticated session.', 'The documentation page itself is read-only.'], check: 'If you are returned to login, sign in again and reopen the original URL.' },
    ...
    preview: { purpose: 'This section explains the read-only notification preview and helper links.', when: 'Use it to review a destination and message before sending.', steps: ['Select a Task Type in the Production notification preview.', 'Check the destination Discord Channel, notification language, and mention state.', 'Review the rendered Discord message and any Kitsu or storage link.'], after: 'The preview only displays the result; it never sends a Discord message.', notes: ['Links are shown only when the configured host and stable IDs are available.', 'Preview data comes from Kitsu and the Production settings.'], check: 'If the destination is empty, review Notification Routing. If a resource is stale, use Troubleshooting.' },
  ...
  return (
    <div className="site">
      <aside className="nav">
        ...
        {navigation('navgroup')}
        <div className="meta">v0.4.0<br />Product guide</div>
      </aside>
      ...
      <main className="body">
        ...
        <footer className="foot">
          <div className="left">KitsuSync Documentation v0.4.0</div>
