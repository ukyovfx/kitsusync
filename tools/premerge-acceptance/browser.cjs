...
async function record(page, pr, route, locale, viewport, state, evidence) {
  records.push({ candidate: process.env.KITSUSYNC_ACCEPTANCE_CANDIDATE_SHA || 'local-unset', pr, route, locale, viewport, fixture_state: state, result: 'PASS', evidence });
}
...
(async () => {
  ...
  try {
    ...
    const readCounts = async () => {
      const response = await page.request.get(`${base}/__fixture?scenario=counts`);
      if (!response.ok()) throw new Error('could not read non-secret fixture counters');
      return response.json();
    };
    const beforeRevalidation = await readCounts();
    await page.goto(`${base}/bot/setup?lang=en&action=review&project=synthetic-production-1&plan_guild=11111111111111111`, { waitUntil: 'networkidle' });
    const reviewForm = page.locator('form.wizard-confirm-form');
    if (await reviewForm.count() !== 1) throw new Error('Production Setup review form did not render');
    await page.locator('#wizard-confirm').check();
    await reviewForm.locator('input[name="action"]').evaluate(input => { input.value = 'prepare_production_connection'; });
    await Promise.all([
    ...
    if (afterRevalidation.project_reads <= beforeRevalidation.project_reads || afterRevalidation.task_type_reads <= beforeRevalidation.task_type_reads) {
      throw new Error('Production Setup revalidation did not repeat persisted-source Production and Task Type reads');
    }
    await record(page, 207, '/bot/setup?wizard_step=5 -> POST prepare -> POST execute', 'en', 'desktop', 'synthetic review and execution with Discord writes stubbed to SQLite-only fixture', 'revalidation performed fresh persisted-source Production and Task Type reads; execution completed without external Discord writes');
    await fixture(page, 'kitsu_empty');
    await page.goto(`${base}/bot/setup?lang=en&wizard_step=2`, { waitUntil: 'networkidle' });
    const emptyProductionText = await page.locator('main').innerText();
    if (!(await page.locator('.state-explanation[role="status"]').filter({ hasText: 'Kitsu returned no Productions' }).count())) throw new Error('empty Production result did not render its successful-empty status');
    await fixture(page, 'kitsu_failure');
    await page.goto(`${base}/bot/setup?lang=en&wizard_step=2`, { waitUntil: 'networkidle' });
    const failedProductionText = await page.locator('main').innerText();
    if (!(await page.locator('.state-explanation[role="alert"]').filter({ hasText: 'Production list could not be loaded from Kitsu' }).count())) throw new Error('Production request failure did not render an alert');
    ...
    await fixture(page, 'multiple_guilds');
    await page.setViewportSize({ width: 1440, height: 1000 });
    await page.goto(`${base}/bot/admin/users?lang=en`, { waitUntil: 'networkidle' });
    await Promise.all([
      page.waitForURL(url => url.searchParams.get('discord_guild_id') === '22222222222222222'),
      page.locator('#global-discord-guild').selectOption('22222222222222222'),
    ]);
    await bodyHas(page, 'Synthetic Human');
    if (await page.locator('.user-linking-table').count() !== 1) throw new Error('explicit guild selection did not load its human member table');
    await record(page, 206, '/bot/admin/users', 'en', 'desktop', 'explicit second synthetic guild selection', 'multiple guild selector navigated to the selected guild and loaded human mappings');
